...
### Phase 2 progress (initial implementation added)

- Added a reusable Go bot-fleet runner under `trading-platform/backend/internal/botfleet`.
- Added strategy generation for `bbo_heavy`, `flash_crash`, `high_cancel`, and `wide_spread`.
- Added a backend `/stress-test` endpoint to launch HTTP or TCP load tests.
- Added a CLI entry point at `trading-platform/backend/cmd/bot-fleet` for running stress tests from the terminal.
