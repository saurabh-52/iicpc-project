...
func main() {
	...
	protocol := flag.String("protocol", "http", "Protocol: http, tcp, or fix")
	strategy := flag.String("strategy", "bbo_heavy", "Strategy: bbo_heavy, flash_crash, high_cancel, wide_spread, market_maker, iceberg, momentum_burst")
	...
	expectReply := flag.Bool("expect-reply", false, "Wait for newline replies in TCP/FIX mode")
	rampUp := flag.Duration("ramp-up", 0, "Gradually add bots over this duration (e.g. 5s)")
	flag.Parse()
